...
const validName = (a) => {
    ...
    .map((el,ind) => {
        ...
        else if(el.length > 2 && el.charAt(el.length - 1) === ".")
        {
            result = false;
        }
        else if(el.charCodeAt(0) < 60 || el.charCodeAt(0) > 90)
        {
            result = false;
        }
        else if(b[0].length === 2 && b[0].charAt(1) === "." && ind === 1 && el.length > 2 && b.length > 2)
        {
            result = false;
        }
        else if(ind === b.length - 1 && el.length <= 2 && el.charAt(el.length - 1) === ".")
        {
            result = false;
        }
        else
